<template>
  <div>
    ...
    <main>
      ...
      <div v-else-if="!movies.length" class="no-results">No movies found!</div>
        ...
        <div v-for="movie in movies" :key="movie.imdbID" class="movie-card">
          <router-link
            :to="{
              ...
              query: { movie_id: movie.imdbID, movie_name: movie.name },
            }"
          >
            <img :src="movie.Poster" :alt="movie.Title" />
            <a
              :href="`http://www.omdbapi.com/?i=${movie.imdbID}&apikey=27f5875a`"
              target="_blank"
            ></a>
            <a> {{ movie.Title }}</a>
            <p>{{ movie.Year }}</p>
